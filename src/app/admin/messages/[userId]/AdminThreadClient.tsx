...
import { sendAdminMessage, type Message } from "@/lib/messages-actions";
...
export default function AdminThreadClient({
  ...
  function send(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!body.trim()) return;
    start(async () => {
      const r = await sendAdminMessage({ userId, body });
  ...
  return (
    <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
      ...
      >
        ...
        <div className={`text-[10px] mt-1 ${mine ? "text-black/60" : "text-buzz-mute"}`}>
          {mine ? "You" : "Them"} ·{" "}
