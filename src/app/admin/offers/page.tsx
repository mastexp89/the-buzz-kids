...
export default async function OffersAdminPage() {
  ...
  const [{ data: offers }, { data: cities }] = await Promise.all([
    supabase.from("offers").select("id, category, title, provider, terms, url, scope, city_id, approved, reports, submitted_email").order("approved").order("category").order("sort_order"),
